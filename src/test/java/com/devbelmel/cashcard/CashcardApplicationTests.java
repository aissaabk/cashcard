package com.devbelmel.cashcard;
...
class CashCardApplicationTests {
	...
	void shouldCreateANewCashCard() {
		CashCard newCashCard = new CashCard(null, 250.00, null);		ResponseEntity<Void> createResponse = restTemplate.withBasicAuth("sarah1", "abc123").postForEntity("/cashcards", newCashCard, Void.class);
    ...
    */
	...
	/**
	 * Let's write a test that asserts that sarah1 isn't allowed to update one of
	 * kumar2's Cash Cards.
	 */
	@Test
	 ...
	 */
	@Test
	void shouldNotAllowDeleteCashCardTheyDoNotOwn(){
		CashCard kumarsCard = new CashCard(null, 333.33, null);
		HttpEntity<CashCard> request = new HttpEntity<>(kumarsCard);
		ResponseEntity<Void> response = restTemplate
	...
	void shouldNotDeleteACashCardThatDoesNotExist() {
		...
		ResponseEntity<Void> response = restTemplate
				.withBasicAuth("sarah1", "abc123")
				.exchange("/cashcards/999", HttpMethod.DELETE, request, Void.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
	}
}
